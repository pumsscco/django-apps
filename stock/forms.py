...
class StockForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        stamps = cleaned_data.get("stamps")
    ...
    #首先是股票代码，必须满足全数字，且长度为6位，而且本人操作的全部为6或0开头的主板股票
        ...
        if balance!=Stock.objects.filter(code=code)[0].balance+volume:
            msg = "变动后的持股数量输入有误"
        ...
        if operation in (u'证券买入',u'证券卖出',u'申购中签'):
            if not float(turnover)==abs(float(price)*volume):
                msg = "成交金额计算错误，请自查"
        ...
        #印花税计算
        if operation==u'证券卖出':
            if not float(stamps)==round(turnover/1000,2):
                msg = "印花税计算错误，请自查"
                self.add_error('stamps', msg)
        else:
            if not stamps==0:
                msg = "非卖出股票不收印花税"
                self.add_error('stamps', msg)
        ...
        if operation==u'证券买入':
            ...
            if code.startswith('6'):
                if not (float(amount)==float(turnover-brokerage-stamps-transfer_fee)):
                    msg = "沪市卖出时，发生金额为成交-佣金-印花税-过户费"
                    self.add_error('amount', msg)
            elif code.startswith('0'):
                if not (float(amount)==float(turnover-brokerage-stamps)):
